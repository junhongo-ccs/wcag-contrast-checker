...
![プラグインのスクリーンショット](screenshot.png)
...
プラグインは、選択されたテキストレイヤーの親ノード（フレーム、グループ、シェイプなど）を順番に遡り、**最初に見つかった有効な塗り（fill）を持つレイヤー**を背景として認識します。
...
この場合、テキストの直接の親であるフレームBには塗りがないため、さらに親を遡ってフレームAの白色が背景として使用されます。

**注意**: 背景がグラデーションや画像の場合はエラーメッセージが表示されます。ソリッドカラー（単色）のみ対応しています。
...
このプラグインはFigma Communityで公開予定です。
